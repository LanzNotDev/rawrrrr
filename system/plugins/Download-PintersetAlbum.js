...
const handler = async (m, { Putzz, text, command }) => {
    if (!text || !text.includes(','))
   return m.reply(`Example: .${command} mikasa,10`)
let [text1, text2] = text.split(',')
  ...
  try {
    const url =
      ...
      encodeURIComponent(
        JSON.stringify({
          options: { query: encodeURIComponent(text1) }
        })
      )
  ...
  const mediaList = []
  for (let url of urls) {
    if (mediaList.length >= text2) break
    try {
  ...
  const opener = generateWAMessageFromContent(
    ...
    {
      userJid: jidNormalizedUser(Putzz.user.id),
      quoted: m,
      upload: Putzz.waUploadToServer
    }
  )

  await Putzz.relayMessage(opener.key.remoteJid, opener.message, {
  ...
  await Putzz.sendMessage(m.chat, { react: { text: null, key: m.key } })
}

handler.help = ["pinalbum"]
handler.tags = ["search", "image"]
handler.command = ["pinalbum", "pin"]
