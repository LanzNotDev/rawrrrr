...
let handler = async (m, { Putzz, command }) => {
  try {
    ...
    // Panggil API FAA ToPeci
    const apiUrl = `https://api-faa.my.id/faa/topeci?url=${encodeURIComponent(imageUrl)}`
    ...
    await Putzz.sendMessage(
      m.chat,
      { image: resultBuffer, caption: '✨ Hasil AI To Peci' },
      { quoted: m }
    )
...
handler.help = ['topeci', 'pecikan', 'jadipeci']
handler.tags = ['ai', 'premium', 'tools']
handler.command = ['topeci', 'pecikan', 'jadipeci']
