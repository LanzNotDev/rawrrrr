...
async function LoadDataBase(Putzz, m) {
	try {
...
async function MessagesUpsert(Putzz, message, store) {
    try {
        ...
        if (!Putzz.public && !msg.key.fromMe && message.type === 'notify') return;

		if (msg.key && msg.key.remoteJid === 'status@broadcast') {
    		if (global.db.settings.readsw && global.db.settings.readsw == true) {
        		Putzz.readMessages([msg.key])
    		} else return;
		}

        if (global.db.settings.autoread && global.db.settings.autoread == true) Putzz.readMessages([msg.key]);
        
        if (global.db.settings.autotyping && global.db.settings.autotyping == true && !msg.key.fromMe) Putzz.sendPresenceUpdate('composing', msg.key.remoteJid);
        
        const m = await Serialize(Putzz, msg, store);
        if (m.isBaileys) return;
        
        require(`../system/Putzz.js`)(Putzz, m, message, store);
  ...
  m.message?.audioMessage?.contextInfo?.quotedMessage?.conversation ||
  m.message?.stickerMessage?.contextInfo?.quotedMessage?.conversation ||
  m.message?.contactMessage?.displayName ||
  m.message?.locationMessage?.name ||
  m.message?.liveLocationMessage?.caption ||
  m.message?.buttonsResponseMessage?.selectedButtonId ||
  m.message?.listResponseMessage?.singleSelectReply?.selectedRowId ||
  m.message?.templateButtonReplyMessage?.selectedId ||
  m.message?.pollCreationMessage?.name ||
  m.message?.pollCreationMessageV3?.name ||
  m.message?.pollUpdateMessage?.name ||
  ...
  m.text || '';
        ...
        let prefixnya = (() => {
            ...
            if (global.multiprefix === true) {
                const found = global.prefixes.find(p => body.startsWith(p));
                return found || null;
            } else if (global.multiprefix === false) {
                return body.startsWith(global.satuprefix) ? global.satuprefix : null;
            } else {
                return null;
            }
        })();

        const prefix = `${prefixnya}`;
        ...
        if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
            ...
            apb.key = msg.key;
            apb.key.fromMe = areJidsSameUser(m.sender, Putzz.user.id);
            ...
            Putzz.ev.emit('messages.upsert', pbr);
        }
...
async function MessagesUpsert2(Putzz, message, store) {
	try {
		let botNumber = await Putzz.decodeJid(Putzz.user.id);
		const msg = message.messages[0];
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
...
async function Solving(Putzz, store) {
	Putzz.public = true
	
	Putzz.serializeM = (m) => MessagesUpsert(Putzz, m, store)
	...
	Putzz.getName = (jid, withoutContact  = false) => {
		const id = Putzz.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || Putzz.groupMetadata(id) || {};
	...
	Putzz.sendContactV2 = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
let list = []
for (let i of kon) {
list.push({
...
Putzz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
	
	Putzz.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await Putzz.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Putzz.getName(i + '@s.whatsapp.net')}\nFN:${await Putzz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Putzz.getName(i + '@s.whatsapp.net')}\nFN:${await Putzz.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/Putzz_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		Putzz.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	}
	...
	Putzz.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await Putzz.query({
			tag: 'iq',
			attrs: {
	...
	Putzz.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Putzz.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return Putzz.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('image')) {
				return Putzz.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('video')) {
				return Putzz.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime && mime.includes('audio')) {
				return Putzz.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	Putzz.sendTextMentions = async (jid, text, quoted, options = {}) => Putzz.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	Putzz.sendAsSticker = async (jid, path, quoted, options = {}) => {
	 ...
	 if (options && (options.packname || options.author)) {
		...
		await Putzz.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted });
		return buff;
	}
	
	
	    Putzz.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
	...
	Putzz.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	Putzz.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Putzz.getFile(path, true);
		...
		await Putzz.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
...
async function Serialize(Putzz, m, store) {
	const botNumber = await Putzz.decodeJid(Putzz.user.id)
	...
	if (m.key) {
		...
		m.sender = await Putzz.decodeJid(m.fromMe && Putzz.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = m.isGroup ? (await Putzz.groupMetadata(m.chat).catch(_ => [{}]) || [{}]) : [{}]
...
if (m.message) {
  ...
  if (m.isMedia) {
    ...
    m.height = m.msg?.height || 0
    m.width = m.msg?.width || 0
    ...
    if (/webp/i.test(m.mime)) {
      m.isAnimated = !!m.msg?.isAnimated
		...
		if (m.quoted) {
			...
			m.quoted.sender = Putzz.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === Putzz.decodeJid(Putzz.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, Putzz)
				return await Serialize(Putzz, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(Putzz.decodeJid(m.msg?.contextInfo?.participant), Putzz.decodeJid(Putzz?.user?.id)),
			...
			m.quoted.delete = () => {
				Putzz.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(Putzz, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return Putzz.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return Putzz.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return Putzz.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
