...
async function mediafire(url) {
  try {
    const safeUrl = encodeURI(url) // 🔥 INI KUNCINYA

    let { data } = await axios.get(safeUrl)
    let $ = cheerio.load(data)

    let link = $('#downloadButton').attr('href')
    if (!link) return { status: false }

    let size = $('#downloadButton').text().match(/\((.*?)\)/)?.[1] || 'Unknown'
    let upload_date = $('ul.details li').eq(1).find('span').text() || '-'

    let fileName = decodeURIComponent(link.split('/').pop())
...
async function mediafiree(query) {
