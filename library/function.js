...
// function.js
async function checkBotAdmin(Putzz, m) {
  try {
    const metadata = await Putzz.groupMetadata(m.chat)
    const botNumber = Putzz.user.id.split(':')[0] + '@s.whatsapp.net'
    const isAdmin = metadata.participants.find(p => p.id === botNumber)?.admin
    return !isAdmin
  } catch (e) {
    console.error('Gagal cek bot admin:', e)
    return false
  }
}

// function.js
function jidEndsWith(jid, suffix) {
  try {
    // Kalau jid berupa object { id: '...' }, ambil field id-nya
    if (typeof jid === 'object' && jid.id) jid = jid.id

    // Pastikan jid berupa string
    if (typeof jid !== 'string') return false

    return jid.endsWith(suffix)
  } catch (e) {
    console.error('Error di jidEndsWith:', e)
    return false
  }
}

//module.exports = { jidEndsWith }



module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, getAllHTML, toIDR, capital, randomToken, checkBotAdmin, jidEndsWith};
